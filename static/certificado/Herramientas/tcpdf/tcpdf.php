			...
			case 'img': {
				if (!empty($tag['attribute']['src'])) {
					if ($tag['attribute']['src'][0] === '@') {
						...
						$type = '';
					} else if (preg_match('/^data:image\/([^;]+);base64,/', $tag['attribute']['src'], $m) > 0) {
						// data stream
						$tag['attribute']['src'] = '@'.base64_decode(substr($tag['attribute']['src'], strlen($m[0])));
						// get image type from MIME string
						$type = $m[1];
						if ($type == 'svg+xml') {
							$type = 'svg';
						}
